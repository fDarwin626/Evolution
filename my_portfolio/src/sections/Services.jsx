...
/* ─── SplitHeader ─── */
const SplitHeader = memo(({ text, style, delay = 0, triggerRef, chrome = false }) => {
  ...
  return (
    ...
    >
      {text.split("").map((ch, i) => (
        <span
          key={i}
          className={chrome ? "sh-char sh-chrome" : "sh-char"}
...
tailored to your business needs.`;
  ...
  useGSAP(() => {
    ...
    // One-time scan sweep, plays each time the section enters view — like a
    // file being pulled up on screen, not a persistent scanline effect
    if (scanRef.current) {
      ...
      tl.set(scanRef.current, { top: "-4%", opacity: 0 })
        .to(scanRef.current, { opacity: 0.55, duration: 0.25 })
  ...
  return (
    <section
      ref={sectionRef}
      id="Services"
      className="bg-black text-white overflow-hidden rounded-t-2xl sm:rounded-t-3xl lg:rounded-t-[2rem] relative"
    ...
    >
      <style>{`
        ...
        .srv-item-row:hover .srv-item-arrow {
          animation: glitchFlicker 0.35s steps(2, end) 1;
      ...
      {/* HUD corner brackets — thin silver frame, the "you're being tracked" cue */}
      ...
      <div ref={headerRef} className="px-5 pt-8 pb-2 lg:px-16 lg:pt-14" style={{ position: "relative", zIndex: 1 }}>
        <SplitHeader
          text="What I"
        ...
        {/* Badge */}
        <div className="srv-reveal glitch-hover mt-5 inline-flex items-center gap-2 border border-white/[0.12] px-3 py-1.5 mb-8">
      ...
      {/* ── Marquee ── */}
      <div className="overflow-hidden border-y border-white/[0.06] py-3 whitespace-nowrap mb-0" style={{ position: "relative", zIndex: 1 }}>
            ...
            <div>
              {service.items.map((item, itemIndex) => (
                <div key={`item-${index}-${itemIndex}`} className="srv-item-row">
                  ...
                  <span className="srv-item-arrow" style={{ color: "rgba(255,255,255,0.12)", fontSize: "10px", flexShrink: 0, paddingTop: "2px", transition: "color 0.2s" }}>↗</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── Bottom CTA strip ── */}
      <div className="px-5 py-8 flex items-center justify-between flex-wrap gap-4 lg:px-16" style={{ position: "relative", zIndex: 1 }}>
        <div>
